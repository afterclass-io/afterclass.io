 ...
 * BOSS round ordering.
 *
 * Verbatim copy of `src/modules/bidding/utils/round-order.ts` — the SINGLE
 * source of truth for round sort order. Rounds not in this map sort to the
 * end in alphabetical order, so new BOSS rounds don't break the chart — they
 * just appear at the end until the map is updated.
 *
 * This ordering has been stable across 5+ years of SMU history (2021–2026).
 * The 7 rounds are: 1, 1A, 1B, 1C, 1F, 2, 2A.
 *
 * Lives here dependency-free (no `@/` imports) so `views/bid-explorer`
 * can import it relatively instead of carrying a copied site.
 */
