...
describe("registerViewlessTools", () => {
  ...
  it("viewBoundNames is derived from ToolRefs, not hardcoded literals", () => {
    // Pins the no-literal-names invariant without importing the real view-tools
  ...
  // MCP kill-switch: mcpEnabled=false refuses at the MCP transport
  ...
  it("viewless tools return raw text envelope (no structuredContent)", async () => {
    ...
    registerViewlessTools({ tool } as never);
    const result = await captured[0]!({}, { auth: { user: { id: "u1" } } });
    // Dispatch wraps success text in <tool_output> delimiters; the
  ...
  describe("destructive confirm gate", () => {
    ...
    // Tier 1: only destructive/high-impact tools are confirm-gated;
    // Tier-2 constructive writes are budget-only (pinned ungated below).
    it.each([
      "remove-timetable",
    ...
    it.each([
      ...
      "copy-public-roadmap",
    ])("constructive %s is budget-only: no confirm gate", async (name) => {
      // Tier 2: constructive writes are NEVER confirm-gated — the
      // gate returns null even without confirm:true, and the handler runs.
      expect(checkDestructiveConfirm(name, {})).toBeNull();
      expect(checkDestructiveConfirm(name, { confirm: true })).toBeNull();
      vi.stubEnv("MCP_DEV_BYPASS", "");
      const run = vi.fn().mockResolvedValue(okText("written"));
      const restore = withDestructiveTool(run, name);
