 ...
 *
 * Canonical strip helper — do NOT build a second one: `stripSecrets` from
 * `@/mcp/output-policy`. Bearer tokens (`shareToken`,
 ...
 * the getter — the old literal is gone, so `tools.ts` and
...
/**
 * Shared cursor pagination over an in-memory list: the cursor is
...
/**
 * Wrap model-visible tool OUTPUT text in `<tool_output>` delimiters.
 * Applied once at the dispatch text-shape success path, so both
