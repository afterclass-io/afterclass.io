...
// Shared lookup + named-throw: same title/annotations/
...
export const searchCourses = server.tool(
  ...
  async (params, ctx) => {
    ...
    const out = await dispatchToolCall({
      ...
      params,
      ctx,
      // No budget: the bespoke adapter went buildToolContext → run with no
      // budget charge (read-budget policy for view tools is set here).
