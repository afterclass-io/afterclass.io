...
describe("search-courses adapter", () => {
  ...
  it("happy path — catalog array becomes {results} structuredContent, summary carries codes", async () => {
    ...
    expect(res.content[0]?.text).toBe(
      "Found 1 courses:\nACCT102 | Management Accounting | 1 sections\nSearch page: /search?q=acc\nACCT102: /course/ACCT102",
    );
  });

  it("summary lists every hit as code | name | N sections", async () => {
    const { handler } = registration("search-courses");
    const second = {
      ...COURSE,
      id: "c2",
    ...
    toolRun.mockResolvedValue({
      content: [{ type: "text", text: JSON.stringify([COURSE, second]) }],
    });
    const res = await handler({ query: "comp" }, {});
    expect(res.isError).toBeUndefined();
    expect(res.content[0]?.text).toBe(
      "Found 2 courses:\nACCT102 | Management Accounting | 1 sections\nCOR-IS1702 | Computational Thinking | 5 sections\nSearch page: /search?q=comp\nACCT102: /course/ACCT102\nCOR-IS1702: /course/COR-IS1702",
    );
  });

  it("summary links the search page only when hits exceed 5", async () => {
    const { handler } = registration("search-courses");
    // Cycle the 15 real seed course codes (prisma/data/3_courses.json) so the
    // >5-hits branch stays on fixture-fair values instead of invented ACCT10x.
    const realCodes = [
      "ACCT102",
      "IS215",
      "COR-IS1702",
    ...
    const hits = Array.from({ length: 6 }, (_, i) => ({
      ...COURSE,
      id: `c${i}`,
      code: realCodes[i % realCodes.length],
    }));
    toolRun.mockResolvedValue({
  ...
  it("summary encodes the query in the search-page link", async () => {
    const { handler } = registration("search-courses");
    toolRun.mockResolvedValue({
      content: [{ type: "text", text: JSON.stringify([COURSE]) }],
    });
    const res = await handler({ query: "machine learning" }, {});
    expect(res.isError).toBeUndefined();
    expect(res.content[0]?.text).toContain(
      "\nSearch page: /search?q=machine%20learning",
    );
  });
  ...
  it("masks a non-array success payload to {results: []} (documented masking)", async () => {
    const { handler } = registration("search-courses");
    toolRun.mockResolvedValue({
      content: [{ type: "text", text: JSON.stringify({ oops: true }) }],
    });
    const res = await handler({ query: "acc" }, {});
    expect(res.isError).toBeUndefined();
  ...
  it("empty results keep the bare count summary plus the search-page link", async () => {
    const { handler } = registration("search-courses");
    toolRun.mockResolvedValue({
    ...
    expect(res.isError).toBeUndefined();
    expect(res.content[0]?.text).toBe(
      "Found 0 courses\nSearch page: /search?q=zzz-no-match",
    );
  });
  ...
  // MCP kill-switch: mcpEnabled=false refuses before run.
