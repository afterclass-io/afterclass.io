...
export interface DispatchPolicy {
   ...
   * When set, `params.confirmToken` is verified (binding
...
export async function dispatchToolCall(opts: {
  ...
  if (policy.budget !== "none") {
    // Single-owner budget: dispatch owns the charge via the single
  ...
  // Write audit: every successful write-tool execution on every
  ...
  // `<tool_output>` injection-boundary delimiters (success text
