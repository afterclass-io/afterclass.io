...
describe("resolveMcpUser", () => {
  ...
  it("resolves by Supabase user id (v2 shape: auth.user.id)", async () => {
    ...
    await expect(resolveMcpUser({ user: { id: "supa-1", email: "a@x.com", amr: [] } as never })).resolves.toMatchObject({ id: "supa-1", email: "a@x.com" });
    expect(usersFindUnique).toHaveBeenCalledWith({ where: { id: "supa-1" } });
  });

  it("still falls back to legacy userId for test callers", async () => {
    usersFindUnique.mockResolvedValue(row);
    await expect(resolveMcpUser({ userId: "supa-1" } as never)).resolves.toMatchObject({ id: "supa-1" });
    expect(usersFindUnique).toHaveBeenCalledWith({ where: { id: "supa-1" } });
  });

  it("resolves by legacy userId (original test path)", async () => {
    usersFindUnique.mockResolvedValue(row);
    await expect(resolveMcpUser({ userId: "supa-1" } as never)).resolves.toMatchObject({ id: "supa-1", email: "a@x.com" });
    expect(usersFindUnique).toHaveBeenCalledWith({ where: { id: "supa-1" } });
  });

  it("prefers v2 id over legacy userId when both present", async () => {
    usersFindUnique.mockResolvedValue(row);
    await expect(resolveMcpUser({ user: { id: "supa-1", amr: [] } as never, userId: "ghost" } as never)).resolves.toMatchObject({ id: "supa-1" });
    expect(usersFindUnique).toHaveBeenCalledWith({ where: { id: "supa-1" } });
  ...
  it("falls back to email when id lookup misses (verified email) via v2 shape", async () => {
    ...
    await expect(resolveMcpUser({ user: { id: "ghost", email: "a@x.com", amr: [] } as never, email_verified: true } as never)).resolves.toMatchObject({ id: "supa-1" });
  ...
  it("falls back to email when legacy id lookup misses (verified email)", async () => {
    usersFindUnique.mockResolvedValueOnce(null).mockResolvedValueOnce(row);
    await expect(resolveMcpUser({ userId: "ghost", email: "a@x.com", email_verified: true } as never)).resolves.toMatchObject({ id: "supa-1" });
    expect(usersFindUnique).toHaveBeenNthCalledWith(2, { where: { email: "a@x.com" } });
  });

  it("falls back to email via email_confirmed_at when email_verified is absent", async () => {
    usersFindUnique.mockResolvedValueOnce(null).mockResolvedValueOnce(row);
    await expect(resolveMcpUser({ userId: "ghost", email: "a@x.com", email_confirmed_at: "2026-01-01T00:00:00Z" } as never)).resolves.toMatchObject({ id: "supa-1" });
    expect(usersFindUnique).toHaveBeenNthCalledWith(2, { where: { email: "a@x.com" } });
  });

  it("does NOT fall back to email when the token email is unverified", async () => {
    usersFindUnique.mockResolvedValueOnce(null);
    await expect(resolveMcpUser({ userId: "ghost", email: "a@x.com" } as never)).resolves.toBeUndefined();
    ...
    usersFindUnique.mockResolvedValueOnce(null);
    await expect(resolveMcpUser({ userId: "ghost", email: "a@x.com", email_verified: false } as never)).resolves.toBeUndefined();
    expect(usersFindUnique).toHaveBeenCalledTimes(1);

    usersFindUnique.mockReset();
    usersFindUnique.mockResolvedValueOnce(null);
    await expect(resolveMcpUser({ email: "a@x.com", email_verified: false } as never)).resolves.toBeUndefined();
  ...
  it("does NOT fall back to v2 email when unverified (Supabase never verified)", async () => {
    usersFindUnique.mockResolvedValueOnce(null);
    await expect(resolveMcpUser({ user: { id: "ghost", email: "a@x.com", amr: [] } as never } as never)).resolves.toBeUndefined();
    expect(usersFindUnique).toHaveBeenCalledTimes(1);
    expect(usersFindUnique).toHaveBeenCalledWith({ where: { id: "ghost" } });
  });
  ...
  it("returns undefined when the user does not exist", async () => {
    ...
    await expect(resolveMcpUser({ userId: "ghost" } as never)).resolves.toBeUndefined();
  });

  it("returns undefined with no identity", async () => {
    await expect(resolveMcpUser({} as never)).resolves.toBeUndefined();
    expect(usersFindUnique).not.toHaveBeenCalled();
  });
