 ...
 * symbols: the app resolves top-level `zod@4.5.4` (single `bun.lock` entry,
 * no `overrides` block), while the SDK bundles its own zod copy
 * (`node_modules/@modelcontextprotocol/server/node_modules/zod` exists).
 * The bundled copy is not governed by our dependency graph so it cannot be
 * collapsed — the `unknown` hop stays: it documents the
