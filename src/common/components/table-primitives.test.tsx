...
describe("SortableTh icon contract — lucide arrows", () => {
  it("renders ArrowUp for active asc", () => {
    ...
    const { container } = render(<SortableTh label="Seats" active dir="asc" onClick={onClick} />);
  ...
  it("renders ArrowDown for active desc", () => {
    const onClick = () => undefined;
    const { container } = render(<SortableTh label="Seats" active dir="desc" onClick={onClick} />);
    expect(container.querySelector("svg")).toBeTruthy();
  });

  it("renders neutral ArrowUpDown for inactive (opacity-50)", () => {
    const onClick = () => undefined;
    const { container } = render(<SortableTh label="Seats" active={false} dir="asc" onClick={onClick} />);
