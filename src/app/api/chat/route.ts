...
// cap does NOT apply to our Pro plan). Next.js requires a LITERAL
...
// Input caps: a pasted/attack payload is refused BEFORE quota,
...
// below); the 0.5 × maxInputTokens derivation is kept
...
export async function POST(req: Request) {
  ...
  // Kill-switch: chatEnabled=false refuses the whole route
  ...
  // Caps: at most MAX_CHAT_MESSAGES turns and MAX_CHAT_BODY_BYTES
  ...
  try {
    ...
    const result = streamText({
      ...
      // Structured per-step usage log: one JSON line per agent-loop
