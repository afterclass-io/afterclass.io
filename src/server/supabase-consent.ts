...
/**
 * HMAC synchronizer CSRF tokens for the OAuth consent round-trip
 * (stateless, no DB row): GET issues a token bound to the sha256 hash of
 * this session's Supabase access token + expiry; POST verifies it before
 * approve/deny. The raw bearer is never embedded — the hex digest is
 * dot-free, stable per session, and reveals nothing. Same shape as the
 * confirm-token helper (user-scoped HMAC, base64url, timing-safe compare).
 * Secret mirrors `supabase-access-token.ts` (`NEXTAUTH_SECRET`, falling
 * back to `AUTH_SECRET`).
 */
...
export function issueConsentCsrf(accessToken: string, secret: string): string {
  const binding = createHash("sha256").update(accessToken).digest("hex");
  const exp = Date.now() + CONSENT_CSRF_TTL_MS;
  const body = `${binding}.${exp}`;
...
export function verifyConsentCsrf(
  ...
  try {
    ...
    // Parse exp/sig as the last two dot-segments so dots in the bound
    // key (e.g. a raw JWT) can never break verification.
    const parts = raw.split(".");
    const sig = parts.pop();
    const expS = parts.pop();
    const bound = parts.join(".");
