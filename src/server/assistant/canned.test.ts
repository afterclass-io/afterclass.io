...
describe("canned", () => {
  it("normalizes punctuation, case, and whitespace", () => {
    expect(normalizePrompt("What can you do??  NOW")).toBe(
      "what can you do now",
    );
  });
  it("finds a canned answer for the capabilities question", () => {
    expect(
      findCannedAnswer([userMsg("What are your capabilities?")]),
    ).toBeTruthy();
  ...
  it("matches the first welcome suggestion prompt as an exact canned phrase", () => {
    ...
    expect(
      findCannedAnswer([
        userMsg("What are your capabilities? What can you help me with?"),
      ]),
    ).toBeTruthy();
  });
  it("does NOT short-circuit on a free-typed near-miss - it must reach the model", () => {
    ...
    expect(
      findCannedAnswer([userMsg("What can you do about my timetable?")]),
    ).toBeNull();
    expect(
      findCannedAnswer([userMsg("what can you do for CS101 bidding")]),
    ).toBeNull();
  });
  it("redirects an exact-match off-topic prompt to afterclass topics", () => {
    const answer = findCannedAnswer([userMsg("Reverse a linked list!")]);
    expect(answer).toContain("afterclass.io assistant");
    expect(answer).toContain("courses");
  });
  it("does NOT redirect a near-miss containing the off-topic phrase", () => {
    expect(
      findCannedAnswer([
        userMsg("Can you reverse a linked list in Python for my homework"),
      ]),
    ).toBeNull();
  });
