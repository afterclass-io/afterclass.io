...
// status.ts reads the canonical chat-config directly.
...
describe("getAssistantStatus", () => {
  beforeEach(() => {
    ...
    // Default: consented (matches the chat-route tests, which assume a
  ...
  // Kill-switch flags surface through status (mcpEnabled stays
  ...
  // aiConsented is consent-date-null → false (fail-closed), date → true.
