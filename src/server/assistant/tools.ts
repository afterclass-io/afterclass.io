 ...
 * the getter. Kept exported (module-scope getter calls) so
...
export function buildAssistantTools(
  ...
  for (const t of allTools) {
        ...
        execute: async (args) => {
          ...
          const out = await dispatchToolCall({
            ...
            policy: {
              ...
              devBypass: false,
              throwBehavior: "propagate",
              // Token pass-through: the model forwards a confirmToken
    ...
    register(t.name);
    const snake = t.name.replace(/-/g, "_");
    if (snake !== t.name) register(snake);
  }
  return tools;
}
