...
vi.mock("@/server/db", () => ({
  ...
  // reserveMessage/settleUsage run interactive $transactions on txDb.
  txDb: {
    $transaction: (fn: (tx: Record<string, unknown>) => unknown) => fn(tx),
  },
}));
// quota.ts reads the canonical chat-config (mocked here); the
// ecfg shim mock stays for modules that still import it transitively.
// beginTurn reads the sync getter for inFlightStaleMs — mocked too.
