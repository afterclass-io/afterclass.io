...
export function isLlmConfigured(e: LlmEnvLike = process.env as LlmEnvLike): boolean {
  ...
  if (!ok && !warned) {
    ...
    console.error("[assistant] LLM_API_KEY missing — chat/MCP-AI disabled, app continues");
  }
  return ok;
}
