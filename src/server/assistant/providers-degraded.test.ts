...
/**
 * Degraded-mode contract (Task 1): without `LLM_API_KEY` the app degrades
 * instead of dying at boot. `isLlmConfigured` is the never-throwing gate
 * (route 503 + status `aiDegraded`); `resolveLlmEnv` stays fail-closed at the
 * per-turn `getModel()` call site so a misconfiguration is still loud there.
 */
