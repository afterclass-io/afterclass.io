...
describe("upsert-roadmap-entry", () => {
  it("is not read-only, exposes roadmap-view View, and adds an entry additively without wiping others", async () => {
    ...
    const saveEntries = vi
      .fn()
      .mockResolvedValue({ count: 3, updatedAt: new Date().toISOString() });
    ...
    const saved = saveEntries.mock.calls[0]![0] as {
      roadmapId: string;
      entries: Array<{ courseId: string }>;
    };
    expect(saved.roadmapId).toBe("r1");
    // Additive: all prior entries preserved plus the new one
    expect(saved.entries.map((e) => e.courseId).sort()).toEqual(
      ["c1", "c2", "c3"].sort(),
    );
    const parsed = JSON.parse(res.content[0]!.text) as {
      roadmapView: { roadmap: { id: string } };
      feasibility: { issues: unknown[]; isFeasible: boolean };
    };
  ...
  it("moves a course already in the roadmap (updates placement instead of duplicating)", async () => {
    ...
    const getMineAfter = vi.fn().mockResolvedValue({
      ...
      entries: [
        entry("c1", "COR-IS1702", "Comp Thinking", 1, "T1", 0),
        entry("c2", "ACCT102", "Management Accounting", 3, "T1", 1),
      ],
    });
    let idx = 0;
    ...
    expect(res.isError).toBeFalsy();
    const saved = saveEntries.mock.calls[0]![0] as {
      entries: Array<{ courseId: string; yearNumber: number; term: string }>;
    };
  ...
  it("returns errText when the user has no roadmaps and no roadmapId was given", async () => {
    const caller = makeCaller({
      ...
      coursesGetByCourseCode: vi
        .fn()
        .mockResolvedValue({ id: "c1", code: "ACCT102", name: "x" }),
    });
    const ctx: ToolContext = { user: fakeUser, caller };
    const res = await upsertRoadmapEntryTool.run(ctx, {
      courseCode: "ACCT102",
  ...
  it("returns errText when saveEntries rejects (e.g. CONFLICT)", async () => {
    const caller = makeCaller({
      ...
      roadmapsSaveEntries: vi
        .fn()
        .mockRejectedValue(new Error("CONFLICT: roadmap updated elsewhere")),
      coursesGetByCourseCode: vi
        .fn()
        .mockResolvedValue({
          id: "c3",
  ...
  it("includes feasibility with PREREQ_MISSING when the new course's prereq is not planned earlier", async () => {
    ...
    expect(res.isError).toBeFalsy();
    const parsed = JSON.parse(res.content[0]!.text) as {
      feasibility: { issues: Array<{ type: string }>; isFeasible: boolean };
    };
    expect(
      parsed.feasibility.issues.some((i) => i.type === "PREREQ_MISSING"),
    ).toBe(true);
  });
  ...
  it("defaults sortOrder to next slot in the target year/term", async () => {
    ...
    const ctx: ToolContext = { user: fakeUser, caller };
    await upsertRoadmapEntryTool.run(ctx, {
      roadmapId: "r1",
      courseCode: "ACCT102",
      yearNumber: 3,
      term: "T1",
    ...
    const saved = saveEntries.mock.calls[0]![0] as {
      entries: Array<{ courseId: string; sortOrder: number }>;
    };
