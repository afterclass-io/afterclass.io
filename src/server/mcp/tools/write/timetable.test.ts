...
describe("timetable write tools", () => {
  it("create-timetable calls timetable.create", async () => {
    ...
    const ctx: ToolContext = {
      user: fakeUser,
      caller: makeCaller({ timetableCreate: fn }),
    };
  ...
  it("create-timetable returns errText when timetable.create rejects", async () => {
    ...
    const result = await createTimetableTool.run(ctx, {
      acadTermId: "t1",
      name: "Plan A",
    });
  ...
  it("add-class-to-timetable calls timetable.addSlot", async () => {
    ...
    const ctx: ToolContext = {
      user: fakeUser,
      caller: makeCaller({ timetableAddSlot: fn }),
    };
    await addClassToTimetableTool.run(ctx, {
      timetableId: "tt1",
      classId: "cl1",
    });
  ...
  it("add-class-to-timetable returns errText when timetable.addSlot rejects", async () => {
    ...
    const result = await addClassToTimetableTool.run(ctx, {
      timetableId: "tt1",
      classId: "cl1",
    });
    expect(result.isError).toBe(true);
  ...
  it("add-class-to-timetable with classId only resolves the active timetable for the class term", async () => {
    ...
    expect(addSlot).toHaveBeenCalledWith({
  ...
  it("add-class-to-timetable with classId only creates a timetable when none exists", async () => {
    ...
    const create = vi
      .fn()
      .mockResolvedValue({ id: "tt-new", acadTermId: "t1" });
    const getAll = vi.fn().mockResolvedValue([{ id: "cl1", acadTermId: "t1" }]);
    const ctx: ToolContext = {
      user: fakeUser,
      caller: makeCaller({
  ...
  it("remove-class-from-timetable calls timetable.removeSlot", async () => {
    ...
    const ctx: ToolContext = {
      user: fakeUser,
      caller: makeCaller({ timetableRemoveSlot: fn }),
    };
    await removeClassFromTimetableTool.run(ctx, {
      timetableId: "tt1",
      classId: "cl1",
    });
    expect(fn).toHaveBeenCalledWith({ timetableId: "tt1", classId: "cl1" });
  ...
  it("remove-class-from-timetable returns errText when timetable.removeSlot rejects", async () => {
    ...
    const result = await removeClassFromTimetableTool.run(ctx, {
      timetableId: "tt1",
      classId: "cl1",
    });
    expect(result.isError).toBe(true);
  ...
  it("rename-timetable calls timetable.rename", async () => {
    ...
    const ctx: ToolContext = {
      user: fakeUser,
      caller: makeCaller({ timetableRename: fn }),
    };
  ...
  it("rename-timetable returns errText when timetable.rename rejects", async () => {
    ...
    const result = await renameTimetableTool.run(ctx, {
      timetableId: "tt1",
      name: "New",
    });
    expect(result.isError).toBe(true);
  ...
  it("remove-timetable calls timetable.remove", async () => {
    ...
    const ctx: ToolContext = {
      user: fakeUser,
      caller: makeCaller({ timetableRemove: fn }),
    };
  ...
  it("set-timetable-visibility calls sharing.setVisibility with entity=timetable", async () => {
    ...
    await setTimetableVisibilityTool.run(ctx, {
      timetableId: "tt1",
      visibility: "UNLISTED",
    });
  ...
  it("set-timetable-visibility returns errText when sharing.setVisibility rejects", async () => {
    ...
    const result = await setTimetableVisibilityTool.run(ctx, {
      timetableId: "tt1",
      visibility: "UNLISTED",
    });
    expect(result.isError).toBe(true);
  ...
  it("set-timetable-visibility strips shareToken bearer token from the output", async () => {
    const fn = vi
      .fn()
      .mockResolvedValue({ visibility: "UNLISTED", shareToken: "secret-tok" });
    const ctx: ToolContext = {
      user: fakeUser,
      caller: makeCaller({ sharingSetVisibility: fn }),
    };
  ...
  it("create-timetable strips shareToken + icalToken bearer tokens from the output", async () => {
    const fn = vi
      .fn()
      .mockResolvedValue({ id: "tt1", shareToken: "s", icalToken: "i" });
    const ctx: ToolContext = {
      user: fakeUser,
      caller: makeCaller({ timetableCreate: fn }),
    };
