...
export const searchCoursesTool: McpTool<typeof searchCoursesSchema> = {
  ...
  toViewProps: (result) => {
    ...
    // Fail loud on parse failure: return { raw } so the adapter
...
export const getClassesTool: McpTool<typeof getClassesSchema> = {
  ...
  run: async ({ caller }, input) => {
    try {
      ...
      // This stays inline instead of pageByCursor — the clamped router fetch
      // needs the OPTIMISTIC rule (a full page
