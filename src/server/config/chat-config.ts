/**
 * Centralized chat/bid/catalog tunables.
 *
 ...
 * | maxInputTokens | 64000 | live ecfg value (config.json still carries the stale 16000; env override documented in .env.example) |
 * | maxOutputTokens | 4096 | live ecfg value (config.json stale 1024 — same note) |
 * | maxToolRounds | 12 | live ecfg value (config.json stale 6 — same note) |
 * | settlementSpikeTokens | 30000 | named tunable; current code derives threshold = floor(maxInputTokens*0.5) capped by this value |
 ...
 * | minBid | 10 | `MIN_BID` in `src/server/mcp/tools/bid-shared.ts` |
...
export const chatConfigSchema = z.object({
  ...
  // Kill-switches: ecfg-owned ONLY — deliberately NO ENV_BINDINGS
 ...
 * CHAT_QUOTA_PER_MONTH-style aliases are accepted at lower precedence. */
const ENV_BINDINGS = [
...
/**
 * Canonical bid floors for zod schemas and hot paths: the
 * mirror literals in `bid-shared.ts` / `write/bids.ts` /
...
/**
 * Canonical tool-output budget: maxChars + truncation note for
 ...
 * Central config read: this module IS the single place raw
 * CHAT_* env reads live, alongside env.ts (schema) and env-gate.ts (gate).
 * Precedence: env-override-then-fallback; fail-closed throws
 ...
 * Central config read (see getChatWriteRateLimit above).
