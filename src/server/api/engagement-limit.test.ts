...
describe("checkEngagementBudget", () => {
  afterEach(() => resetLimits());

  it("allows up to the limit then blocks within the window", () => {
    vi.useFakeTimers();
    const key = "r1:127.0.0.1";
    expect(checkEngagementBudget(key, 3, 60_000)).toBe(true);
    expect(checkEngagementBudget(key, 3, 60_000)).toBe(true);
    expect(checkEngagementBudget(key, 3, 60_000)).toBe(true);
    expect(checkEngagementBudget(key, 3, 60_000)).toBe(false);
  });

  it("resets after the window elapses", () => {
    vi.useFakeTimers();
    const key = "r1:127.0.0.1";
    checkEngagementBudget(key, 3, 60_000);
  ...
  it("prunes expired entries when Map reaches MAX_BUCKETS", () => {
    ...
    for (let i = 0; i < 1000; i++) {
      checkEngagementBudget(`key-${i}`, 1, 1000);
    }
    ...
    checkEngagementBudget("fresh", 1, 60_000);
    ...
    expect(checkEngagementBudget("key-0", 1, 60_000)).toBe(true);
    vi.useRealTimers();
  });
