...
beforeEach(() => {
  renameMock = vi.fn((_id: string, _title: string): Promise<void> =>
    Promise.resolve(),
  );
...
function renderList() {
  ...
  const onNew = vi.fn();
  render(
    <SessionList activeSessionId="s1" onSelect={onSelect} onNew={onNew} />,
  );
  return { onSelect, onNew };
}

describe("SessionList a11y - non-nested interactives + focus/touch-visible actions", () => {
  ...
  it("highlights the active session on its select button", () => {
    ...
    expect(
      screen
        .getByRole("button", { name: "Math homework" })
        .getAttribute("aria-current"),
    ).toBe("true");
    expect(
      screen
        .getByRole("button", { name: "Essay draft" })
        .getAttribute("aria-current"),
    ).toBeNull();
  });
  ...
  it("renames a session through the store", async () => {
    renderList();
    fireEvent.click(
      screen.getByRole("button", { name: /^rename math homework$/i }),
    );
    const input = screen.getByLabelText("Session title");
    fireEvent.change(input, { target: { value: "Physics notes" } });
    fireEvent.keyDown(input, { key: "Enter", code: "Enter" });
    await waitFor(() =>
      expect(renameMock).toHaveBeenCalledWith("s1", "Physics notes"),
    );
  });
  ...
  it("keeps the inline editor OUTSIDE any button, and hides action buttons during edit mode", () => {
    ...
    expect(
      screen.queryByRole("button", { name: /^rename math homework$/i }),
    ).toBeNull();
    expect(
      screen.queryByRole("button", { name: /^delete math homework$/i }),
    ).toBeNull();
...
describe("SessionList delete - AlertDialog confirm (no native alert)", () => {
  ...
  it("opens an AlertDialog on trash click without calling window.confirm", async () => {
    ...
    expect(
      within(dialog).getByText(/This will permanently delete/i),
    ).toBeInTheDocument();
    expect(within(dialog).getByText(/Math homework/)).toBeInTheDocument();
    expect(
      screen.getByTestId("assistant-session-delete-confirm"),
    ).toBeInTheDocument();
  ...
  it("keeps the session row in its normal state while the dialog is open", async () => {
    ...
    expect(
      screen.getByRole("button", { name: "Math homework", hidden: true }),
    ).toBeInTheDocument();
    expect(
  ...
  it("removes the session when Delete is confirmed in the dialog", async () => {
    ...
    fireEvent.click(
      await screen.findByTestId("assistant-session-delete-confirm"),
    );
    await waitFor(() => expect(deleteMock).toHaveBeenCalledWith("s1"));
  });
  ...
  it("closes without removing on Cancel", async () => {
    ...
    const cancelBtn = Array.from(
      getDialogEls()!.querySelectorAll("button"),
