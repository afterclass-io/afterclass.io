...
describe("widget-geometry", () => {
  it("clamps position inside the viewport with a margin", () => {
    ...
    expect(clampPosition({ x: -50, y: 10_000 }, size, viewport)).toEqual({
      x: 8,
      y: 800 - 560 - 8,
    });
  });
  ...
  it("clamps size to min/max bounds", () => {
    expect(clampSize({ width: 100, height: 5000 })).toEqual({
      width: MIN_WIDGET_SIZE.width,
      height: MAX_WIDGET_SIZE.height,
    });
  });
  ...
  it("applies resize deltas and clamps to the minimum size", () => {
    expect(applyResize(MIN_WIDGET_SIZE, { x: -200, y: -200 })).toEqual(
      MIN_WIDGET_SIZE,
    );
  });
  ...
  it("clamps the derived box inside the viewport on tiny screens", () => {
    ...
    const box = boxPositionFromLauncher(
      { x: 8, y: 8 },
      { width: 400, height: 560 },
      viewport,
    );
