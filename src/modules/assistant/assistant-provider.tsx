...
type Status =
  ...
  | {
      ...
      nudgeAt: number;
      aiDegraded: boolean;
      // Kill-switch: present on fresh status payloads; absent on stale/cached
      // payloads (treated as enabled — fail-open for cached status so a
      // stale fetch cannot hide the widget).
      chatEnabled?: boolean;
      widgetEnabled?: boolean;
      // Consent flag, same staleness story — but fail-CLOSED: absent
...
function SignedInAssistant({
  ...
  // Consent is LOCAL to the widget mount — seeded from the status
  ...
  // Kill-switch: widget fully hidden when disabled. Anonymous branch
...
export function AssistantProvider() {
  ...
  // Refetch every time the widget OPENS (in addition to route
