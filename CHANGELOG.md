...
- Assistant / MCP (feat/mcp-ai wave 2026-09-01): multi-step assistant budgets raised (tool rounds 12, output tokens 4096, duration 300), prompt steering (clarify/ask-back, stay in-scope, typo retry, term/window defaults) + multi-step orchestration guidance; current-term/open-window server-side defaults for tool schemas; search hardening (min-length, word_similarity, prefix FTS, code/professor normalization) + `search-professors`; search now also matches description/courseArea with a faculty filter; `get-classes` exposes day/startsAfter/endsBefore; `get-my-timetable-detail` exposes exam timings; new tools: `get-review-summary`, `bid-estimate`, `save-bids` (bulk, 1 write token), `upsert-roadmap-entry` (additive); bid/budget and roadmap writes echo the full updated plan/roadmap (`{ updated, plan }`) and re-render their widgets; widget CTA buttons show transient Done/Error feedback and only render when the host can call tools; catalog now 50 tools / 7 widgets.
- Assistant AI cost hardening (2026-09-01): provider-native `prompt_cache_hit_tokens` mapped to settlement (`cachedInput` now real, not 0); `abortSignal` wired to client disconnect so Stop stops output-token billing; tool-result clamp at the chat choke point (24k chars); review defaults 10/max 20 + `limit` params on `get-bid-results`/`my-bids`; restored sessions drop tool parts instead of hollow shells; cache-hit rate surfaced in assistant status; `CHAT_LOG_USAGE=1` diagnostic + settlement-spike warning.
...
- MCP server migrated to mcp-use v2 (`mcp-use` 1.34.6 → 2.3.4, `@mcp-use/cli` 3.6.7 → 4.1.8; Node >=22.22.2). The ESM import is now `from "mcp-use"` (the `mcp-use/server` subpath is gone). MCP Apps **widgets → Views**: `src/mcp/resources/<name>/widget.tsx` → `views/<name>/view.tsx`; `widget:{name}` → `view:{name}`; `useWidget` → `useToolContext` / `useHostContext` / `useDynamicTool` / `useViewTheme`; mcp-use server helpers (result wrappers, `widget()`) removed in favor of raw MCP wire envelopes `{ content, structuredContent, _meta }`. Tool registration: 7 view-bound tools registered with `outputSchema` + exported ToolRefs from `src/mcp/view-tools/*` (one View per canonical tool — search-courses, get-timetable-calendar-link, my-bid-plan, get-my-roadmap, get-course-reviews, explore-bid-options, get-my-timetable-detail); the other 43 tools are viewless loop-registered returning raw results (no wrapper View dirs; `recommend-bid-amount` is viewless — its view was removed — so it registers with the 43). View assets are served under `_mcp-use/views/...` and the Inspector moved to `/mcp/inspector` (basePath default `/mcp` + `/inspector`).
- MCP auth: Supabase OAuth provider now imported from `mcp-use/oauth/supabase` (`oauthSupabaseProvider({ projectId, supabaseUrl, jwtSecret })`, env vars passed explicitly from `src/mcp/server.ts`; dev still omits OAuth entirely, prod fails fast on missing project config). Tool context reads the user from `ctx.auth.user.id` (v1 `ctx.userId` is gone). View-bound tool results are validated against zod `outputSchema`s before reaching Views; `get-timetable-calendar-link` still keeps bearer iCal URLs out of model context, now via the View-only `_meta` channel.
- MCP review hardening (2026-09-03 → 2026-09-06 wave): `confirm:true` gates extended — calendar-link escalation, roadmap budget/visibility writes (`set-bid-budget`, `set-roadmap-visibility` etc. gated alongside the destructive deletes); MCP read budget (per-user read cap alongside the write budget); public selects drop `userId`/`isActive` (no user linkage or activity flags leak through public catalog reads); Sentry `attachRpcInput` off (no RPC args captured in error events); consent GET route guarded (no unauthenticated detail fetch); `check-roadmap-feasibility` extracted into its own tool module; bid floor via a shared helper; views consolidated on the shared kit (`views/shared/` tokens + skeleton + CTA feedback); null-budget nudge in `my-bid-plan` (prompts the user to set a budget instead of rendering an empty plan); course descriptions surfaced in search/detail payloads; roadmap progress (`sync-roadmap-progress`) surfaced; `plan-semester` (plan-term) prompt steering tightened; page-context widget sends the current route + course/section/classId/prof with each chat turn; fixture seed-real sweep (course codes/names re-pinned to real seed values across view stories/tests/adapters); story coverage gaps filled (day normalization, timetable exam rows, nullable branches). **[IN PROGRESS this session — docs/hygiene + story-gaps lane: plan-term prompt + page-context widget + shared view components; fixture sweep and story gaps landed in the fidelity workstreams.]**
...
- feat(ui): add Google Auth as sign in option by [@kKar1503](https://github.com/kKar1503) in [#387](https://github.com/afterclass-io/afterclass.io/pull/387)
- feat(ui): sort and filter review items by [@davidlhw](https://github.com/davidlhw) in [#390](https://github.com/afterclass-io/afterclass.io/pull/390)
- feat(courses): add course_area and enrolment_requirements fields by [@tanzhongyan](https://github.com/tanzhongyan) in [#404](https://github.com/afterclass-io/afterclass.io/pull/404)
- feat(flags): use feature flag for review reactions by [@davidlhw](https://github.com/davidlhw) in [#406](https://github.com/afterclass-io/afterclass.io/pull/406)

### 👾 Bug Fixes

- fix: auth display error on failure & session user to match db user on google auth by [@davidlhw](https://github.com/davidlhw) in [#389](https://github.com/afterclass-io/afterclass.io/pull/389)

### Other Changes

- feat(ui): chart component with shadcn + recharts by [@davidlhw](https://github.com/davidlhw) in [#376](https://github.com/afterclass-io/afterclass.io/pull/376)
- feat(ui): track review analytics with events by [@davidlhw](https://github.com/davidlhw) in [#382](https://github.com/afterclass-io/afterclass.io/pull/382)
- chore(devconfig): ESLint & TSConfig issues by [@kKar1503](https://github.com/kKar1503) in [#388](https://github.com/afterclass-io/afterclass.io/pull/388)
- chore: remove posthog dependencies by [@davidlhw](https://github.com/davidlhw) in [#391](https://github.com/afterclass-io/afterclass.io/pull/391)
- feat: wrap review events tracking behind feature flags on edge config by [@davidlhw](https://github.com/davidlhw) in [#392](https://github.com/afterclass-io/afterclass.io/pull/392)
- chore: pin bun & node version by [@davidlhw](https://github.com/davidlhw) in [#395](https://github.com/afterclass-io/afterclass.io/pull/395)
- refactor(api): extract queries into logical components by [@davidlhw](https://github.com/davidlhw) in [#398](https://github.com/afterclass-io/afterclass.io/pull/398)
- chore(deps): upgrade prisma to v6 by [@davidlhw](https://github.com/davidlhw) in [#396](https://github.com/afterclass-io/afterclass.io/pull/396)
- feat(ui): add review item footer with vote group and view count by [@davidlhw](https://github.com/davidlhw) in [#401](https://github.com/afterclass-io/afterclass.io/pull/401)
- feat(ui): add hovercard shadcn component by [@davidlhw](https://github.com/davidlhw) in [#402](https://github.com/afterclass-io/afterclass.io/pull/402)
- fix(review-events): add tracking for downvotes by [@davidlhw](https://github.com/davidlhw) in [#403](https://github.com/afterclass-io/afterclass.io/pull/403)
- feat(ui): add review reactions by [@davidlhw](https://github.com/davidlhw) in [#405](https://github.com/afterclass-io/afterclass.io/pull/405)
- refactor: remove triggeringUserId as a param to track & vote api by [@davidlhw](https://github.com/davidlhw) in [#409](https://github.com/afterclass-io/afterclass.io/pull/409)
- style: layout fixes for sidebar inset & review submission by [@davidlhw](https://github.com/davidlhw) in [#408](https://github.com/afterclass-io/afterclass.io/pull/408)
- refactor: use hooks for reaction mutation with optimistic updates by [@davidlhw](https://github.com/davidlhw) in [#410](https://github.com/afterclass-io/afterclass.io/pull/410)
- feat(db): add db tables for boss data by [@tanzhongyan](https://github.com/tanzhongyan) in [#411](https://github.com/afterclass-io/afterclass.io/pull/411)

## New Contributors

- [@kKar1503](https://github.com/kKar1503) made their first contribution in [#387](https://github.com/afterclass-io/afterclass.io/pull/387)
- [@tanzhongyan](https://github.com/tanzhongyan) made their first contribution in [#404](https://github.com/afterclass-io/afterclass.io/pull/404)

**Full Changelog**: https://github.com/afterclass-io/afterclass.io/compare/v1.3.0...v1.4.0
