## [Unreleased]
### Changed
- MCP server migrated to mcp-use v2 (`mcp-use` 1.34.6 → 2.3.4, `@mcp-use/cli` 3.6.7 → 4.1.8; Node >=22.22.2). The ESM import is now `from "mcp-use"` (the `mcp-use/server` subpath is gone). MCP Apps **widgets → Views**: `src/mcp/resources/<name>/widget.tsx` → `src/mcp/views/<name>/view.tsx`; `widget:{name}` → `view:{name}`; `useWidget` → `useToolContext` / `useHostContext` / `useDynamicTool` / `useViewTheme`; mcp-use server helpers (result wrappers, `widget()`) removed in favor of raw MCP wire envelopes `{ content, structuredContent, _meta }`. Tool registration: 7 view-bound tools registered with `outputSchema` + exported ToolRefs from `src/mcp/view-tools/*` (one View per canonical tool — course-search, bid-recommendation, calendar-links, bid-plan, roadmap-view, review-cards, bid-explorer); the other 42 tools are viewless loop-registered returning raw results (no wrapper View dirs). View assets are served under `_mcp-use/views/...` and the Inspector moved to `/mcp/inspector` (basePath default `/mcp` + `/inspector`).
